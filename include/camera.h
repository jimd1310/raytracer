...
public: 
    ...
    int max_depth         = 10;     // Maxmimum number of ray bounces into scene
...
private:
    ...
    ray get_ray(int i, int j) const {
        // Construct camera ray originating from origin directed at randomly 
    ...
    color ray_color(const ray& r, int depth, const hittable& world) {
        // If we've exceeded ray bounce limit, not more light is gathered
        if (depth <= 0)
            return color(0, 0, 0);
